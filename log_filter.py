...
class LogFilter(object):
    ...
    def load_json(data):
        ...
        if 'name' in data:
            log_filter.set_name(data['name'])
        else:
            log_filter.set_name(None)

        if 'include' in data:
            log_filter.set_include(data['include'])
        else:
            log_filter.set_include({})

        if 'exclude' in data:
            log_filter.set_exclude(data['exclude'])
        else:
            log_filter.set_exclude({})
        return log_filter
